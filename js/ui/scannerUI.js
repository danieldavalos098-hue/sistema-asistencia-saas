...
export function showScanResult(student, group, mode, code, time = '') {
  ...
  if (!student) {
    if (avatar)  { avatar.textContent = '?'; avatar.style.background = '#ef4444'; avatar.style.color = '#fff'; }
    if (nameEl)  nameEl.textContent  = `Código: ${code}`;
    if (groupEl) groupEl.textContent = 'Alumno no registrado';
    if (typeEl)  { typeEl.textContent = 'No encontrado'; typeEl.style.color = '#ef4444'; typeEl.style.fontWeight = '700'; }
    if (timeEl)  timeEl.textContent  = '';
  } else {
    const av = getAvatarStyle(student.name);
    if (avatar)  { avatar.textContent = av.initials; avatar.style.background = av.bg; avatar.style.color = av.color; }
    if (nameEl)  nameEl.textContent  = `${student.name} ${student.lastname || ''}`.trim();
    if (groupEl) groupEl.textContent = group?.name || 'Sin grupo';
    if (typeEl)  { typeEl.textContent = mode; typeEl.style.color = mode === 'ENTRADA' ? '#00d4aa' : '#f59e0b'; typeEl.style.fontWeight = '700'; }
    if (timeEl)  timeEl.textContent  = time;
  }

  result.style.transform = 'translateY(0)';
  setTimeout(() => { result.style.transform = 'translateY(100%)'; }, 3500);
}
